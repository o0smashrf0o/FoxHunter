...
On the Pi, from the SmashDeck tree:
...
This sets hostname `smashdeck`, boot splash, dashboard systemd service,
...
Flash `dist/pi-gen/deploy/SmashDeckOS-*.img` (or `.zip`) with Raspberry Pi Imager.
...
1. Plymouth splash — **SMASHDECK OS**
2. Autologin `smash`
3. `smashdeck-dashboard` on `:8080`
4. WebKit kiosk (Epiphany/GTK), Chromium only if WebKit is missing

Esc leaves the kiosk. `sudo systemctl status smashdeck-dashboard`
